...
int16_t AtFinder::maxTextLength=0;
...
void AtFinder::setup(int16_t _maxTextLength, DISPLAY_CALLBACK _callback) {
...
void AtFinder::processInputChar(char hot) {
  ...
  static int16_t screenId;
  static int16_t screenRow;
  static int16_t textLength;
  ...
  switch(state) {
    ...
    case SKIP_SPACES3:     // skip spaces to "text"
         ...
         else state=FIND_START; 
         return;    
    case COPY_TEXT:          // copying text to buffer
         if (hot=='"') {
          // end of text found
          text[textLength]=0;
          if (callback) callback(screenId, screenRow,text);  // ET PHONE HOME!
          state=FIND_START;
          return; 
         }
         if (textLength<maxTextLength) text[textLength++]=hot;
         return; 
  }
}
